...
type RoomState = {
  ...
  betStatus?: "open" | "locked" | "paid";
  deposits?: Record<string, true>;
  poolTotal?: number;
  createdAt: string;
  updatedAt: string;
};
...
function friendChatKey(a: string, b: string): string {
  return [a, b].sort().join(":");
}

function isUserInRoom(nk: nkruntime.Nakama, userId: string): boolean {
  const list = nk.storageList(SYSTEM_USER_ID, COLLECTION_ROOMS, 100);
  const rooms = list?.objects ?? [];
  return rooms.some((obj) => {
    const room = obj.value as RoomState;
    return Array.isArray(room.members) && room.members.includes(userId);
  });
}

function ensureUser(ctx: nkruntime.Context): string {
  if (!ctx.userId) {
...
function getRoomCapacity(mode: string): number {
  if (mode === "pvp_1v1") {
    return 2;
  }
  return 2;
...
function readRoom(nk: nkruntime.Nakama, roomId: string): { room: RoomState; version: string } {
  ...
  return { room: record.value as RoomState, version: record.version };
}

function writeRoom(nk: nkruntime.Nakama, room: RoomState, version?: string): void {
  nk.storageWrite([
    {
...
function deleteRoom(nk: nkruntime.Nakama, roomId: string): void {
  nk.storageDelete([{ collection: COLLECTION_ROOMS, key: roomId, userId: SYSTEM_USER_ID }]);
}

function updateReadyCooldown(nk: nkruntime.Nakama, userId: string, roomId: string): void {
  const key = `${roomId}:${userId}`;
  const nowMs = Date.now();
  const records = nk.storageRead([{ collection: COLLECTION_READY_CD, key, userId }]);
  if (records.length > 0) {
    const lastAt = (records[0].value as { lastAt: number }).lastAt;
...
function readWallet(nk: nkruntime.Nakama, userId: string): Wallet {
  const records = nk.storageRead([
    { collection: COLLECTION_WALLET, key: "wallet", userId },
  ]);
  if (!records || records.length === 0) {
    const wallet: Wallet = { gold: 0, spoils: {} };
    nk.storageWrite([
      { collection: COLLECTION_WALLET, key: "wallet", userId, value: wallet },
    ]);
    return wallet;
  }
  return records[0].value as Wallet;
}

function writeWallet(nk: nkruntime.Nakama, userId: string, wallet: Wallet): void {
  nk.storageWrite([
    { collection: COLLECTION_WALLET, key: "wallet", userId, value: wallet },
  ]);
}
...
function readPairLosses(
  nk: nkruntime.Nakama,
  userA: string,
  userB: string
): { losses: Record<string, number>; blockedUntil?: number } {
  const key = getPairKey(userA, userB);
  const records = nk.storageRead([{ collection: COLLECTION_PVP_PAIR, key, userId: SYSTEM_USER_ID }]);
  if (!records || records.length === 0) {
    return { losses: {} };
  }
  return records[0].value as { losses: Record<string, number>; blockedUntil?: number };
}

function writePairLosses(
  nk: nkruntime.Nakama,
  userA: string,
  userB: string,
  data: { losses: Record<string, number>; blockedUntil?: number }
): void {
  const key = getPairKey(userA, userB);
  nk.storageWrite([
    {
...
function totp(secret: string, windowSeconds = 30, digits = 6, forTime?: number): string {
  const time = forTime ?? Math.floor(Date.now() / 1000);
...
function otpauthUri(login: string, secret: string): string {
...
function readPending2FA(nk: nkruntime.Nakama, login: string): Pending2FA | null {
  const records = nk.storageRead([
  ...
  return records[0].value as Pending2FA;
}

function writePending2FA(nk: nkruntime.Nakama, pending: Pending2FA): void {
  nk.storageWrite([
    { collection: COLLECTION_AUTH_PENDING, key: pending.login, userId: SYSTEM_USER_ID, value: pending },
  ]);
}

function deletePending2FA(nk: nkruntime.Nakama, login: string): void {
...
function checkRateLimit(nk: nkruntime.Nakama, key: string, limit: number, windowMs: number): void {
  const now = Date.now();
  const records = nk.storageRead([
    { collection: COLLECTION_AUTH_RATE, key, userId: SYSTEM_USER_ID },
  ]);
  let entry: RateEntry = { count: 0, windowStart: now };
  if (records && records.length > 0) {
    entry = records[0].value as RateEntry;
  }
...
function loginExists(nk: nkruntime.Nakama, login: string): boolean {
  const records = nk.storageRead([
...
function saveLoginMapping(nk: nkruntime.Nakama, login: string, userId: string): void {
  nk.storageWrite([
    {
...
function saveUserSecret(nk: nkruntime.Nakama, userId: string, login: string, secret: string): void {
  const record: Stored2FA = { login, secret, createdAt: nowIso() };
  nk.storageWrite([
    { collection: COLLECTION_AUTH_SECRETS, key: userId, userId, value: record },
  ]);
}

function readUserSecret(nk: nkruntime.Nakama, userId: string): Stored2FA | null {
  if (!userId) {
    return null;
  }
  const records = nk.storageRead([
  ...
  return records[0].value as Stored2FA;
}

function readLoginMapping(nk: nkruntime.Nakama, login: string): { login: string; userId: string } | null {
  if (!login) {
    return null;
  }
  const records = nk.storageRead([
  ...
  return records[0].value as { login: string; userId: string };
}

function findUserIdByUsername(nk: nkruntime.Nakama, login: string): string | null {
  try {
...
function ensureAccountForLogin(nk: nkruntime.Nakama, login: string): string {
...
// RPCs
const rpcRegister2FAStart: nkruntime.RpcFunction = (_ctx, _logger, nk, payload) => {
  const data = parsePayload<{ login: string }>(payload);
  if (!data.login || data.login.length < 3) {
    throw new Error("login required");
  }
  checkRateLimit(nk, `start:${data.login}`, 5, 60_000);
...
const rpcRegister2FAVerify: nkruntime.RpcFunction = (ctx, _logger, nk, payload) => {
  const data = parsePayload<{ login: string; code: string }>(payload);
  if (!data.login || !data.code) {
    throw new Error("login and code required");
  }
  const ip = ctx.clientIp || "unknown";
  checkRateLimit(nk, `verify:${data.login}:${ip}`, 8, 60_000);
  const pending = readPending2FA(nk, data.login);
  if (!pending || pending.expiresAt < Date.now()) {
  ...
  if (pending.lastAttemptAt && now - pending.lastAttemptAt < 30_000 && attempts > 5) {
...
const rpcCreateCharacter: nkruntime.RpcFunction = (ctx, _logger, nk, payload) => {
  const userId = ensureUser(ctx);
  const data = parsePayload<{ name: string }>(payload);
  const name = (data.name || "").trim();
  if (!name || name.length < 3) {
    throw new Error("name required");
  }
  const list = nk.storageList(userId, COLLECTION_CHARACTERS, 10);
  const existing = list?.objects ?? [];
...
const rpcDeleteCharacter: nkruntime.RpcFunction = (ctx, _logger, nk, payload) => {
  const userId = ensureUser(ctx);
  const data = parsePayload<{ characterId: string }>(payload);
...
const rpcFriendsList: nkruntime.RpcFunction = (ctx, _logger, nk, _payload) => {
  const userId = ensureUser(ctx);
  const list = nk.storageList(userId, COLLECTION_FRIENDS, 200);
  const friends = list?.objects?.map((obj) => obj.value as FriendEntry) ?? [];
  return JSON.stringify({ friends });
};

const rpcFriendRequests: nkruntime.RpcFunction = (ctx, _logger, nk, _payload) => {
  const userId = ensureUser(ctx);
  const list = nk.storageList(userId, COLLECTION_FRIEND_REQUESTS, 200);
  const requests = list?.objects?.map((obj) => obj.value as FriendRequest) ?? [];
  return JSON.stringify({ requests });
};

const rpcFriendAdd: nkruntime.RpcFunction = (ctx, _logger, nk, payload) => {
  const userId = ensureUser(ctx);
  const data = parsePayload<{ username: string }>(payload);
  ...
  if (incoming && incoming.length > 0) {
    const since = nowIso();
    const entryA: FriendEntry = {
      friendId: target.id,
      friendUsername: target.username ?? "",
      since,
    };
    const entryB: FriendEntry = {
      friendId: userId,
...
const rpcFriendAccept: nkruntime.RpcFunction = (ctx, _logger, nk, payload) => {
  const userId = ensureUser(ctx);
  const data = parsePayload<{ fromUserId: string }>(payload);
  ...
  const request = records[0].value as FriendRequest;
  const since = nowIso();
  const entryA: FriendEntry = {
    friendId: fromUserId,
...
const rpcFriendDecline: nkruntime.RpcFunction = (ctx, _logger, nk, payload) => {
  const userId = ensureUser(ctx);
  const data = parsePayload<{ fromUserId: string }>(payload);
  const fromUserId = (data.fromUserId || "").trim();
  if (!fromUserId) {
...
const rpcFriendsChatSend: nkruntime.RpcFunction = (ctx, _logger, nk, payload) => {
  const userId = ensureUser(ctx);
  const data = parsePayload<{ toUserId: string; text: string }>(payload);
  ...
  const history = (records?.[0]?.value?.messages as FriendChatMessage[]) ?? [];
...
const rpcFriendsChatHistory: nkruntime.RpcFunction = (ctx, _logger, nk, payload) => {
  const userId = ensureUser(ctx);
  const data = parsePayload<{ withUserId: string; limit?: number }>(payload);
...
const rpcGetMyCharacters: nkruntime.RpcFunction = (ctx, _logger, nk, _payload) => {
  const userId = ensureUser(ctx);
  const list = nk.storageList(userId, COLLECTION_CHARACTERS, 10);
  const characters = list?.objects?.map((obj) => obj.value as Character) ?? [];
  return JSON.stringify({ characters });
};

const rpcSelectCharacter: nkruntime.RpcFunction = (ctx, _logger, nk, payload) => {
  const userId = ensureUser(ctx);
  const data = parsePayload<{ characterId: string }>(payload);
  if (!data.characterId) {
    throw new Error("characterId required");
  ...
  const character = records[0].value as Character;
...
const rpcCreateRoom: nkruntime.RpcFunction = (ctx, _logger, nk, payload) => {
  const userId = ensureUser(ctx);
  const data = parsePayload<{ mode: string }>(payload);
  if (!data.mode) {
...
const rpcJoinRoom: nkruntime.RpcFunction = (ctx, _logger, nk, payload) => {
  const userId = ensureUser(ctx);
  const data = parsePayload<{ roomId: string }>(payload);
  if (!data.roomId) {
...
const rpcLeaveRoom: nkruntime.RpcFunction = (ctx, _logger, nk, payload) => {
  const userId = ensureUser(ctx);
  const data = parsePayload<{ roomId: string }>(payload);
  if (!data.roomId) {
    throw new Error("roomId required");
...
const rpcReady: nkruntime.RpcFunction = (ctx, _logger, nk, payload) => {
  const userId = ensureUser(ctx);
  const data = parsePayload<{ roomId: string }>(payload);
  if (!data.roomId) {
    throw new Error("roomId required");
...
const rpcUnready: nkruntime.RpcFunction = (ctx, _logger, nk, payload) => {
  const userId = ensureUser(ctx);
  const data = parsePayload<{ roomId: string }>(payload);
  if (!data.roomId) {
    throw new Error("roomId required");
...
const rpcSetRoomBet: nkruntime.RpcFunction = (ctx, _logger, nk, payload) => {
  const userId = ensureUser(ctx);
...
const rpcDepositBet: nkruntime.RpcFunction = (ctx, _logger, nk, payload) => {
  const userId = ensureUser(ctx);
  const data = parsePayload<{ roomId: string }>(payload);
  if (!data.roomId) {
    throw new Error("roomId required");
  ...
  if (room.bet.type === "gold") {
    ...
    const itemId = room.bet.itemId ?? "";
    const current = wallet.spoils[itemId] ?? 0;
    if (current < amount) {
...
const rpcStartRoomMatch: nkruntime.RpcFunction = (ctx, _logger, nk, payload) => {
  const userId = ensureUser(ctx);
  const data = parsePayload<{ roomId: string }>(payload);
  if (!data.roomId) {
    throw new Error("roomId required");
...
const rpcMarketListItem: nkruntime.RpcFunction = (_ctx, _logger, _nk, _payload) => {
  throw new Error("not implemented");
};
const rpcMarketCancel: nkruntime.RpcFunction = (_ctx, _logger, _nk, _payload) => {
  throw new Error("not implemented");
};
const rpcMarketBuy: nkruntime.RpcFunction = (_ctx, _logger, _nk, _payload) => {
  throw new Error("not implemented");
};
const rpcResetBuildWithGold: nkruntime.RpcFunction = (_ctx, _logger, _nk, _payload) => {
  throw new Error("not implemented");
};
const rpcConvertSpoilsToGold: nkruntime.RpcFunction = (_ctx, _logger, _nk, _payload) => {
  throw new Error("not implemented");
};
const rpcGuildCraft: nkruntime.RpcFunction = (_ctx, _logger, _nk, _payload) => {
  throw new Error("not implemented");
};
const rpcGuildUpgrade: nkruntime.RpcFunction = (_ctx, _logger, _nk, _payload) => {
  throw new Error("not implemented");
};
...
const beforeAuthenticateCustom: nkruntime.BeforeHookFunction<nkruntime.AuthenticateCustomRequest> = (
  ...
  _logger.info(`BeforeAuthCustom create=${req.create} id=${req.id} username=${req.username} accountId=${(req as { account?: { id?: string } }).account?.id}`);
  const create = req.create ?? false;
  const account = (req as { account?: { id?: string; vars?: Record<string, string> } }).account ?? {};
  const vars = req.vars ?? account.vars ?? {};
  const candidates = [req.id, req.username, account.id].filter((value) => Boolean(value)) as string[];
  ...
  let mapping: { login: string; userId: string } | null = null;
...
const afterAuthenticateCustom: nkruntime.AfterHookFunction<nkruntime.AuthenticateCustomRequest> = (
  ctx,
  _logger,
  nk,
  req
  ...
  const create = req.create ?? false;
  _logger.info(`AfterAuthCustom create=${create} id=${req.id} username=${req.username} accountId=${(req as { account?: { id?: string } }).account?.id} userId=${ctx.userId}`);
  const account = (req as { account?: { id?: string } }).account ?? {};
...
// Match handlers
const pveMatch: nkruntime.MatchHandler = {
...
const pvpMatch: nkruntime.MatchHandler = {
  matchInit: (ctx, _logger, _nk, params) => {
    const typedParams = params as MatchParams;
  ...
  matchLoop: (_ctx, logger, nk, dispatcher, tick, state, messages) => {
    ...
    for (const message of messages) {
      ...
      try {
        ...
        logger.warn("invalid input: %s", (error as Error).message);
      }
    }
    ...
    for (const player of Object.values(typed.players)) {
      if (player.left) {
        ...
        typed.winnerId = findOpponent(typed.players, player.userId)?.userId;
        break;
      }
    }
...
function findOpponent(players: Record<string, PlayerState>, userId: string): PlayerState | undefined {
  return Object.values(players).find((player) => player.userId !== userId);
}

function resolveWinnerByDamage(players: Record<string, PlayerState>): string | undefined {
...
function buildSnapshot(
  ...
  tick: number;
  players: Record<string, { x: number; y: number; hp: number; mp: number }>;
  events: Array<{ type: string; payload: unknown }>;
} {
  const view: Record<string, { x: number; y: number; hp: number; mp: number }> = {};
...
function buildBaseMatchSummary(state: PvpState): MatchSummary {
...
function persistMatchSummary(nk: nkruntime.Nakama, summary: MatchSummary): void {
  nk.storageWrite([
    {
...
function buildMatchSummary(
  nk: nkruntime.Nakama,
  state: PvpState,
  antiTrade: MatchSummary["antiTrade"]
): MatchSummary {
...
function applyPayout(nk: nkruntime.Nakama, payout: MatchSummary["payout"]): void {
  if (!payout) {
    return;
  }
  ...
  if (payout.type === "gold") {
    ...
    const itemId = payout.itemId ?? "";
    wallet.spoils[itemId] = (wallet.spoils[itemId] ?? 0) + payout.amount;
  }
  writeWallet(nk, payout.winnerId, wallet);
}

function resolveAntiTrade(nk: nkruntime.Nakama, state: PvpState): MatchSummary["antiTrade"] {
  ...
  const current = pair.losses[loserId] ?? 0;
  const next = current + 1;
  const updated: { losses: Record<string, number>; blockedUntil?: number } = {
